...
class ProgramSarjana extends BaseController
{
    ...
    public function add()
    {
        ...
        $data['prodi_s2'] = $prodiModel->where('jenjang', 'S2')->findAll();
        $data['prodi_s3'] = $prodiModel->where('jenjang', 'S3')->findAll();
        ...
        if (!$input) {
            ...
            if ($idorg) {
                ...
                if ($this->request->getVar('s2_universitas')) {
                    if($this->request->getVar('s2_prodi_alt')){
                        $s2_prodi = $this->request->getVar('s2_prodi_alt');
                    } else {
                        $s2_prodi = $this->request->getVar('s2_prodi');
                    }
                    $s2 = [
                        'jenjang' => 'S2',
                        'idorg' => $idorg,
                ...
                if ($this->request->getVar('s3_universitas')) {
                    if($this->request->getVar('s2_prodi_alt')){
                        $s3_prodi = $this->request->getVar('s3_prodi_alt');
                    } else {
                        $s3_prodi = $this->request->getVar('s3_prodi');
                    }
                    $s3 = [
                        'jenjang' => 'S3',
                        'idorg' => $idorg,
