...
<div class="container-fluid" x-data="ui">
    ...
    <!--begin::Form-->
    <form @submit.prevent='handleSubmit' class="form" method="POST" action="<?= site_url() ?>program_sarjana/edit/<?= $alumni['id'] ?>" id="addForm">
        <?= csrf_field() ?>
        ...
        <!--begin::Actions-->
        <div class="card-footer d-flex justify-content-center py-6" x-data="{loading: false}">
            <button type="submit" class="btn btn-primary" @click="loading=true" x-show="!loading">Save Changes</button>
...
<script>
    document.addEventListener('alpine:init', () => {
        ...
        $(addForm.querySelector('[name="s3_prodi_alt"]')).on('change', function() {
            var s2_prodi = addForm.querySelector('[name="s3_prodi_alt"]').value;
            addForm.querySelector('[name="s3_prodi"]').value = s2_prodi;
        });
