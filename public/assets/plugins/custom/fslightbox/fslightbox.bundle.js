...
! function(e, t) {
    ...
    return function(e) {
        ...
        function z(e, t) {
            ...
            const s = n.sourceMainWrappers;
        
            // Create new source wrapper
            const wrapper = document.createElement("div");
            wrapper.className = `${r}invalid-file-wrapper ${a}`;
        
            const textNode = document.createTextNode("Invalid source");
            wrapper.appendChild(textNode);
        
            o[t] = wrapper;
        
            i[t].classList.add(g);
            i[t].appendChild(wrapper);
        ...
        // }
        function k(e, t) {
            const hideLoaderFn = e.componentsServices.hideSourceLoaderIfNotYetCollection;
            const { sourceWrappersContainer: container, sourceMainWrappers: mainWrappers } = e.elements;
          
            // Buat div utama
            const mainWrapper = document.createElement("div");
            mainWrapper.className = `${d} ${c} ${a}`;
            e.elements.sourceMainWrappers[t] = mainWrapper;
          
            // Buat loader wrapper
            const loaderWrapper = document.createElement("div");
            loaderWrapper.className = "fslightbox-loader";
          
            // Tambahkan 4 div kosong ke dalam loader
            for (let i = 0; i < 4; i++) {
              loaderWrapper.appendChild(document.createElement("div"));
            }
          
            // Tambahkan loader ke dalam wrapper utama
            mainWrapper.appendChild(loaderWrapper);
          
            // Fungsi untuk menghilangkan loader (jika masih ada)
            hideLoaderFn[t] = function () {
              if (mainWrapper.contains(loaderWrapper)) {
                mainWrapper.removeChild(loaderWrapper);
              }
            };
            ...
            // Tambahkan animation wrapper
            (function addAnimationWrapper(e, t) {
              const { sourceMainWrappers, sourceAnimationWrappers } = e.elements;
              const animWrapper = document.createElement("div");
              sourceAnimationWrappers[t] = animWrapper;
              sourceMainWrappers[t].appendChild(animWrapper);
            })(e, t);
          }
        ...
        function R(e) {
            const sources = e.props.sources;
            const container = e.elements.container;
          
            const nav = document.createElement("div");
            nav.className = `${r}nav`;
            container.appendChild(nav);
          
            // Build toolbar (fullscreen + close)
            (function buildToolbar(e, parent) {
              const toolbar = document.createElement("div");
              toolbar.className = `${r}toolbar`;
              parent.appendChild(toolbar);
          
              // Fullscreen Button
              (function addFullscreenButton(e, parent) {
                const services = e.componentsServices;
                const fullscreenToggler = e.core.fullscreenToggler;
                const data = e.data;
                const enterIconPath = "M4.5 11H3v4h4v-1.5H4.5V11zM3 7h1.5V4.5H7V3H3v4zm10.5 6.5H11V15h4v-4h-1.5v2.5zM11 3v1.5h2.5V7H15V3h-4z";
          
                const btn = O(parent); // Assumed: O creates a button element
                btn.title = "Enter fullscreen";
          
                const icon = D(btn, "20px", "0 0 18 18", enterIconPath); // D = create SVG
          
                services.enterFullscreen = function () {
                  data.isFullscreenOpen = true;
                  btn.title = "Exit fullscreen";
                  icon.setAttribute("width", "24px");
                  icon.setAttribute("height", "24px");
                  icon.setAttribute("viewBox", "0 0 950 1024");
                  icon.firstChild.setAttribute("d", "M682 342h128v84h-212v-212h84v128zM598 810v-212h212v84h-128v128h-84zM342 342v-128h84v212h-212v-84h128zM214 682v-84h212v212h-84v-128h-128z");
                };
          
                services.exitFullscreen = function () {
                  data.isFullscreenOpen = false;
                  btn.title = "Enter fullscreen";
                  icon.setAttribute("width", "20px");
                  icon.setAttribute("height", "20px");
                  icon.setAttribute("viewBox", "0 0 18 18");
                  icon.firstChild.setAttribute("d", enterIconPath);
                };
          
                btn.onclick = function () {
                  data.isFullscreenOpen ? fullscreenToggler.exitFullscreen() : fullscreenToggler.enterFullscreen();
                };
              })(e, toolbar);
          
              // Close Button
              (function addCloseButton(e, parent) {
                const btn = O(parent, "Close");
                btn.onclick = e.core.lightboxCloser.closeLightbox;
                D(btn, "20px", "0 0 24 24", "M 4.7070312 3.2929688 L 3.2929688 4.7070312 L 10.585938 12 L 3.2929688 19.292969 L 4.7070312 20.707031 L 12 13.414062 L 19.292969 20.707031 L 20.707031 19.292969 L 13.414062 12 L 20.707031 4.7070312 L 19.292969 3.2929688 L 12 10.585938 L 4.7070312 3.2929688 z");
              })(e, toolbar);
            })(e, nav);
          
            // Slide Number
            if (sources.length > 1) {
              (function buildSlideNumber(e, parent) {
                const services = e.componentsServices;
                const sources = e.props.sources;
          
                const container = document.createElement("div");
                container.className = `${r}slide-number-container`;
          
                const inner = document.createElement("div");
                inner.className = a;
          
                const current = document.createElement("span");
                services.setSlideNumber = (val) => (current.textContent = val);
          
                const slash = document.createElement("span");
                slash.className = `${r}slash`;
                slash.textContent = "/";
                ...
                // Adjust alignment if too long
                setTimeout(() => {
                  if (inner.offsetWidth > 55) {
          ...
          function j(e, t) {
            const mainWrappers = e.elements.sourceMainWrappers;
            const props = e.props;
            let offset = 0;
          
            const applyTransform = (value) => {
              mainWrappers[t].style.transform = `translateX(${value + offset}px)`;
              offset = 0;
            };
          
            const calculateDistance = () => {
              return (1 + props.slideDistance) * window.innerWidth;
            };
          
            this.byValue = function (val) {
              offset = val;
              return this;
            };
          
            this.negative = function () {
              applyTransform(-calculateDistance());
            };
        ...
        function K(e) {
            const { core, elements, componentsServices, props, data, stageIndexes, collections } = e;
          
            // 1. Clean up classes across element arrays
            core.classFacade.removeFromEachElementClassIfContains = (key, className) => {
              const arr = elements[key];
              if (!Array.isArray(arr)) return;
              arr.forEach(el => el.classList.remove(className));
            };
          
            // 2. Event dispatcher mapping to props
            core.eventsDispatcher.dispatch = (eventName) => {
              const handler = props[eventName];
              if (typeof handler === 'function') handler();
            };
          
            // 3. Fullscreen toggler with browser compatibility checks
            core.fullscreenToggler.enterFullscreen = () => {
            ...
            core.fullscreenToggler.exitFullscreen = () => {
            ...
            // 4. Global event listener setup/teardown (keyboard, resize, pointer)
            const evDisp = core.globalEventsController;
            const resize = core.windowResizeActioner;
            const pointerDown = e.resolve.bind(null, U)();
            const pointerMove = e.resolve.bind(null, q)();
            const pointerUp = e.resolve.bind(null, $)();
          
            evDisp.attachListeners = () => {
              document.addEventListener('pointermove', pointerMove.listener);
              document.addEventListener('pointerup', pointerUp.listener);
              window.addEventListener('resize', resize.runActions);
              document.addEventListener('keydown', pointerDown.listener);
            };
            evDisp.removeListeners = () => {
              document.removeEventListener('pointermove', pointerMove.listener);
              document.removeEventListener('pointerup', pointerUp.listener);
              window.removeEventListener('resize', resize.runActions);
              document.removeEventListener('keydown', pointerDown.listener);
            };
          
            // 5. Lightbox close handler
            const stopFade = e.resolve(V);
            core.lightboxCloser.closeLightbox = () => {
              if (!stopFade.isLightboxFadingOut) stopFade.runActions();
            };
          
            // 6. Scrollbar compensation during animation/fullscreen
            core.scrollbarRecompensor.addRecompense = () => {
              const adjust = () => {
                if (document.body.offsetHeight > window.innerHeight) {
                  document.body.style.marginRight = `${data.scrollbarWidth}px`;
                }
              };
              if (document.readyState === 'complete') adjust();
              else window.addEventListener('load', adjust, { once: true });
            };
            core.scrollbarRecompensor.removeRecompense = () => {
              document.body.style.removeProperty('margin-right');
            };
          
            // 7. Navigation change logic based on slide count
            const slideChange = core.slideChangeFacade;
            const slideChanger = core.slideIndexChanger;
            const controller = core.stageManager;
            if (props.sources.length > 1) {
              slideChange.changeToPrevious = () => slideChanger.jumpTo(controller.getPreviousSlideIndex());
              slideChange.changeToNext = () => slideChanger.jumpTo(controller.getNextSlideIndex());
            } else {
              slideChange.changeToPrevious = slideChange.changeToNext = () => {};
            }
          
            // 8. Slide change with animation classes
            const transformers = collections.sourceMainWrappersTransformers;
            const animWrappers = elements.sourceAnimationWrappers;
            const service = componentsServices;
            let clearAnimation;
            const animTimeout = () => {
              clearTimeout(clearAnimation);
              clearAnimation = setTimeout(() => {
                core.classFacade.removeFromEachElementClassIfContains('sourceAnimationWrappers', m);
              }, 300);
            };
          
            slideChanger.changeTo = (idx) => {
              stageIndexes.current = idx;
              controller.updateStageIndexes();
              service.setSlideNumber(idx + 1);
              core.sourceDisplayFacade.displaySourcesWhichShouldBeDisplayed();
            };
          
            slideChanger.jumpTo = (idx) => {
              const prev = stageIndexes.current;
              slideChanger.changeTo(idx);
              core.classFacade.removeFromEachElementClassIfContains('sourceMainWrappers', u);
          
              // Animate out old slide
              animWrappers[prev]?.classList.remove(g, h);
              animWrappers[prev]?.classList.add(m);
          
              if (animWrappers[idx]) {
                animWrappers[idx].classList.remove(g, m);
                animWrappers[idx].classList.add(h);
              }
          
              animTimeout();
              transformers[idx]?.zero();
              setTimeout(() => {
                if (prev !== stageIndexes.current) transformers[prev]?.negative();
              }, 270);
            };
          
            // 9. Pointer-down handling for slide swipes
            const mainWrappers = elements.sourceMainWrappers;
            const sourcesList = elements.sources;
            const pointerProps = e.sourcePointerProps;
            const stageIdx = stageIndexes;
          
            core.sourcesPointerDown.listener = (ev) => {
              if (ev.target.tagName !== 'VIDEO') ev.preventDefault();
              pointerProps.isPointering = true;
              pointerProps.downScreenX = ev.screenX;
              pointerProps.swipedX = 0;
              pointerProps.isSourceDownEventTarget =
                sourcesList[stageIdx.current]?.contains(ev.target) ?? false;
              core.classFacade.removeFromEachElementClassIfContains('sourceMainWrappers', u);
            };
          
            // 10. Lazy display of source elements
            const renderFns = collections.sourcesRenderFunctions;
            core.sourceDisplayFacade.displaySourcesWhichShouldBeDisplayed = () => {
              if (props.loadOnlyCurrentSource) {
                renderFns[stageIdx.current]?.() && delete renderFns[stageIdx.current];
              } else {
                Object.keys(renderFns).forEach(key => {
                  renderFns[key]?.();
                  delete renderFns[key];
                });
              }
            };
          
            // 11. Stage manager navigation logic
            const totalSources = props.sources.length;
            controller.getPreviousSlideIndex = () =>
              stageIdx.current === 0 ? totalSources - 1 : stageIdx.current - 1;
            controller.getNextSlideIndex = () =>
              stageIdx.current === totalSources - 1 ? 0 : stageIdx.current + 1;
            controller.updateStageIndexes = () => {
              if (totalSources <= 1) return;
              if (totalSources === 2) {
                if (stageIdx.current === 0) {
                  stageIdx.next = 1;
                  delete stageIdx.previous;
                } else {
                  stageIdx.previous = 0;
                  delete stageIdx.next;
                }
              } else {
                stageIdx.previous = controller.getPreviousSlideIndex();
                stageIdx.next = controller.getNextSlideIndex();
              }
            };
            controller.isSourceInStage = totalSources <= 2
              ? () => true
              : (idx) => {
                const cur = stageIdx.current;
                return (
                  (cur === 0 && idx === totalSources - 1) ||
                  (cur === totalSources - 1 && idx === 0) ||
                  Math.abs(cur - idx) <= 1
                );
              };
          
            // 12. Responsive sizing and scroll compensation on resize
            const sizers = collections.sourceSizers;
            resize.runActions = () => {
              data.maxSourceWidth = window.innerWidth < 992
                ? window.innerWidth
                : 0.9 * window.innerWidth;
              data.maxSourceHeight = 0.9 * window.innerHeight;
          
              props.sources.forEach((_, idx) => {
                core.classFacade.removeFromEachElementClassIfContains('sourceMainWrappers', u);
                if (idx !== stageIdx.current) transformers[idx]?.negative();
                sizers[idx]?.adjustSize();
              });
            };
          }
        ...
        function Q(e) {
            ...
            n.isInitialized = !0, n.scrollbarWidth = function(e) {
                ...
                }(e), K(e), o.container = document.createElement("div"), o.container.className = "".concat(r, "container ").concat(c, " ").concat(g),
                function(e) {
                    var t = e.elements;
        ...
        function Z(e) {
            ...
            i.open = function() {
                ...
                h.current = o, p.isInitialized ? r.dispatch("onShow") : Q(e), u.updateStageIndexes(), a.displaySourcesWhichShouldBeDisplayed(), n.setSlideNumber(o + 1), document.body.appendChild(f.container), document.documentElement.classList.add(l), c.addRecompense(), s.attachListeners(), d.runActions(), t[h.current].zero(), r.dispatch("onOpen")
            }
        }
